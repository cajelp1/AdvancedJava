...
public class T01_JdbcTest {
	public static void main(String[] args) {
		...
		try {
			...
			// 5.ResultSet객체에 저장되어 있는 자료를 반복문과 next() 메서드를 이용하여 차례로 읽어와 처리
			System.out.println("실행한 쿼리문 : "+sql);
			System.out.println("=== 쿼리문 실행 결과 ===");
			
			
			// rs.next() -> ReseultSet객체의 데이터를 가리키는 포인터를 다음 레코드로 이동시키고
			//				그곳에 자료가 있으면 true, 없으면 false를 리턴한다.
			while(rs.next()) {
				// 컬럼의 자료를 가져오는 방법
				// 방법1. rs.get자료형이름("컬럼명")
				// 방법2. rs.get자료형이름(컬럼번호) => 컬럼번호는 1부터 시작
				System.out.println("lprod_id : "+ rs.getInt("lprod_id")); //"컬럼명"
				System.out.println("lprod_gu : "+ rs.getString(2)); //컬럼번호.. 근데 이건 바뀔 수 있다
