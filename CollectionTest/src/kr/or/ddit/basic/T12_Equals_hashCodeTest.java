...
/*
	...
	객체들이 서로 같은지를 비교하기 위해 equals() 메서드와 hashCode() 메서드를 호출한다.
	...
	HashSet, HashMap, HashTable 에서는 객체가 같은지 여부는 데이터를 추가할 때 검사한다.
		...
		2. 두 객체가 같으면 equals() 메서드를 호출했을 때 true를 반환한다
			( a.eqauls(b) = true; b.equals(a) = true; )
		...
		5. hashCode() 는 기본적으로 Heap에 있는 각 개체에 대한 메모리 주소 매핑정보를 기반으로 한 정수값을 반환한다.
			그러므로 클래스에서 hashCode() 메서드를 override 하지 않으면 절대로 두 객체가 같은 것으로 간주 될 수 없다.
		
		- hashCode() 메서드에서 사용하는 '해싱 알고리즘'에서 서로 다른 객체에 대해 같은 hashCode값을 만들어 낼 수 있다. 
...
class Person {
	...
	// 이거 왜 바뀌는게 없지? 엥.. 그냥 보여주시려구 했다구? ㅡㅡ .... 이해안됨.
	// 이터레이터 안 써도 sysout 하면 그냥 프린트되는데 이터레이터 왜 쓰지? 객체일때 안에 접근하기 위해..?
	@Override
	public int hashCode() {
