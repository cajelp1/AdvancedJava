...
public class MainController implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		try {
			...
			serverAddress = InetAddress.getByName("192.168.205.8"); // 서버주소 설정
			String chatName = showPromptWindow(); // 대화명을 입력받는다.
			
			// 대화명 전송
			DatagramPacket outPacket = 
					new DatagramPacket(chatName.getBytes(), chatName.getBytes().length, serverAddress, 8989);
	...
	public void sendMessage(ActionEvent event){
		...
		DatagramPacket outPacket = new DatagramPacket(message.getBytes(), message.getBytes().length, serverAddress, 8989);
		try {
			socket.send(outPacket);
		} catch (IOException e) {
			e.printStackTrace();
