...
class LockAccount{
	...
	public void deposit(int money) {
		// Lock 객체의 Lock() 메서드가 동기화 시작이고, unlock() 메서드가 동기화 끝을 낸다
	...
	public boolean withdraw(int money) {
		...
		try {
			if(balance >= money) {
				...
				balance -= money;
				System.out.println("메서드 안에서 balace = "+balance);
				
				chk = true;
			}
		}catch(Exception e) {chk = false;}
		finally { lock.unlock();} //동기화 해제
...
class BankThread2 extends Thread{
	...
	public void run() {
		...
		System.out.println("쓰레드 안에서 result = "+result+", balace = "+lAcc.getBalance());
	}
}
