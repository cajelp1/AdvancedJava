...
public class ReviewWrite {
    ...
    @FXML
    void initialize(int res_no) {
    	
    	//세션에서 값 가져와서 세팅
    	id = SessionVO.id;
    	nickname = SessionVO.nickname;
    ...
    void writeArticle(ActionEvent event) {
    	...
    	if(title.getText().trim().isEmpty()) {
    		...
    		vo.setReview_writer(nickname);
    		vo.setReview_writer_id(id);
    		
    		try {
				boolean a = service.insertReview(vo, vo.getRes_no());
				//boolean b = service.reservToComp(vo.getRes_no()); //예약의 상태를 리뷰완료로 바꿔야.
				if(a ) {
					infoMsg("성공","글 작성이 완료되었습니다");
...
//				}
//				else if(a && !b){
//					errMsg("오류", "해당 예약 상태 변경에 실패했습니다");
//				}else if(!a && b) {
//					errMsg("오류", "리뷰 작성 실패. 예약상태만 변경되었습니다. 관리자에게 문의하세요.");
				}else{
					errMsg("오류", "리뷰 작성에 실패했습니다.");
				}
